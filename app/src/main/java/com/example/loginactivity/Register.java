                ...
                fAuth.createUserWithEmailAndPassword(email, password).addOnCompleteListener(new OnCompleteListener<AuthResult>() {
                    ...
                    public void onComplete(@NonNull Task<AuthResult> task) {
                        if(task.isSuccessful()){
                            ...
                            user1 = email;
                            user1 = user1.replace(".", ",");
                            Toast.makeText(Register.this, "User1: " + user1, Toast.LENGTH_SHORT).show();
                            Login.user = user1;

                            rootNode1 = FirebaseDatabase.getInstance();
                            reference2 = rootNode1.getReference("Users");

                            Testing_HelperClass helperClass = new Testing_HelperClass(" ", " ", " ");
                            reference2.child(user1).child(" ").setValue(helperClass);
